...
class VirtualMouseApp(App):
    def __init__(self, **kwargs):
        ...
        self.move_dist = ''  # Variable to store value from first entry field
        self.click_dist = ''
    ...
    def build(self):
        ...
        self.create_movement_field(right_layout)
        self.create_click_field(right_layout)
    ...
    def create_movement_field(self, parent_layout):
        # Second Entry Field
        self.entry_move_field = TextInput(hint_text='Enter Movement Distance(Example: 150)', size_hint=(None, None), size=(250, 50), multiline=False)
        self.entry_move_field.bind(text=self.on_movement_dist_text)  # Bind text change event
        parent_layout.add_widget(self.entry_move_field)

    def create_click_field(self, parent_layout):
        # Second Entry Field
        self.entry_click_field = TextInput(hint_text='Enter Click Distance(Example: 50)', size_hint=(None, None), size=(250, 50), multiline=False)
        self.entry_click_field.bind(text=self.on_click_dist_text)  # Bind text change event
        parent_layout.add_widget(self.entry_click_field)
            ...
            if output.multi_hand_landmarks:
                ...
                for id, landmark in enumerate(landmarks):
                    ...
                    if id == int(self.finger2):
                        ...
                        if distance < int(self.click_dist):
                            pyautogui.click()
                            pyautogui.sleep(0.5)
                        elif distance < int(self.move_dist):
    ...
    def on_movement_dist_text(self, instance, value):
        self.move_dist = value

    def on_click_dist_text(self, instance, value):
        self.click_dist= value
